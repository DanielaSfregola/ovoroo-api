...
import com.ovoenergy.ovoroo.services.AuthenticationService
  ...
  def authRoutes: Route = path("auth") {
      post {
        entity(as[LdapCredentials]) { credentials =>
          ...
          complete(user.map(authenticationService.getOrCreateUserSession))
        }
      }
  }

  private def authenticate(credentials: LdapCredentials): Option[User] = {
    ...
    try {
      ...
      Some(toUser(ldapUser))
    } catch {
      case ex: Exception => None
    }
  }

  private def toUser(ldapUser: LdapUserDetails): User = {
